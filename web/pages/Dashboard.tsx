...
const Dashboard: React.FC = () => {
    ...
    const getStatusRowColor = (status: ElderlyStatus) => {
    ...
    const getStatusBadge = (status: ActivityStatus, isFall: boolean, fallType: FallType) => {
        ...
        const color = getActivityColor(isFall ? ActivityStatus.FALLEN : status);

        const badgeStyle: React.CSSProperties = {
            backgroundColor: color,
            borderColor: color, // You might want a slightly darker/lighter border
            color: 'white', // Assuming white text looks good on all background colors
        };

        if (isFall) {
            return <span className={`${baseClasses} animate-pulse`} style={badgeStyle}>检测到{FALL_TYPE_LABELS[fallType] || '跌倒'}</span>;
        }

        return <span className={baseClasses} style={badgeStyle}>{ACTIVITY_LABELS[status] || '未知状态'}</span>;
    };
            ...
            <div className="flex-1 min-h-0 bg-white rounded-xl shadow-sm border border-slate-100 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 pt-2 space-y-4">
                    {users.map(user => {
                        ...
                        return (
                            <div key={user.id} className={`p-4 rounded-xl border flex flex-col md:flex-row items-center justify-between transition-all hover:shadow-md ${getStatusRowColor(status)}`}>
